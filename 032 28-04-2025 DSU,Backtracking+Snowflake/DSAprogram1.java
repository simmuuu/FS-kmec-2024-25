...
//     // will not always work as we can go below 2 midway
//     // private static int sol(int n, int c, int[] inp) {
//     //     int max = Arrays.stream(inp).max().getAsInt();
//     //     int sum = Arrays.stream(inp).sum();

//     //     return sum - Math.min(max, c) + 1;
//     // }

//     private static int sol(int n, int c, int[] inp) {
//         int l = 1, r = Arrays.stream(inp).sum() + 1;
//         int res = -1;

//         while(l <= r) {
//             int mid = l + (r - l) / 2;

//             if(canVisit(inp, c, mid)) {
//                 res = mid;
//                 r = mid - 1;
//             } else l = mid + 1;
//         }
...
//     private static boolean canVisit(int[] inp, int c, int money) {
//         boolean couponUsed = false;

//         for(int m: inp) {
//             if(money >= m) money -= m;
//             else {
//                 if(!couponUsed && money + Math.min(c, m) >= m) {
//                     money += Math.min(c, m) - m;
//                     couponUsed = true;
//                 } else return false;
//             }
...
//         System.out.println(sol(n, c, inp));
//     }
// }